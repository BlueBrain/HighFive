...
template <typename T, typename U = typename inspector<T>::hdf5_type>
inline void
...
template <typename Container, typename T = typename inspector<Container>::hdf5_type>
...
struct data_converter<
    ...
    typename std::enable_if<(
        std::is_same<T, typename inspector<T>::hdf5_type>::value &&
...
struct data_converter<
    std::array<T, S>,
    typename std::enable_if<(
        std::is_same<T, typename inspector<T>::hdf5_type>::value)>::type>
...
struct data_converter<std::vector<Reference>, void> {
    ...
    std::vector<size_t> _dims;
    std::vector<typename inspector<hobj_ref_t>::hdf5_type> _vec_align;
};
