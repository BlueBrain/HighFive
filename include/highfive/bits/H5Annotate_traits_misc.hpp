...
inline Attribute AnnotateTraits<Derivate>::createAttribute(const std::string & attribute_name, const DataSpace & space, const DataType & dtype) {
  Attribute attribute;
...
inline Attribute AnnotateTraits<Derivate>::createAttribute(const std::string & attribute_name, const DataSpace & space) {
...
inline std::vector<std::string> AnnotateTraits<Derivate>::listAttributeNames() const {
  ...
  if (H5Aiterate2(static_cast<const Derivate*>(this)->getId(), H5_INDEX_NAME,
    H5_ITER_INC, NULL, &details::internal_high_five_iterate<H5A_info_t>, static_cast<void*>(&iterateData)) < 0) {
