...
BOOST_AUTO_TEST_CASE(HighFiveBasic) {
  ...
  BOOST_CHECK_EQUAL(file.getFileName(), FILE_NAME);
...
BOOST_AUTO_TEST_CASE(HighFiveGroupAndDataSet) {
...

  {
    ...
    {
      SilenceHDF5 silencer;
      BOOST_CHECK_THROW(file.createDataSet(CHUNKED_DATASET_NAME, dataspace, AtomicType<double>(),
                                           LinkCreateProps(), badChunking0),
                        DataSetException);

      BOOST_CHECK_THROW(file.createDataSet(CHUNKED_DATASET_NAME, dataspace, AtomicType<double>(),
                                           LinkCreateProps(), badChunking1),
                        DataSetException);
    }
...
BOOST_AUTO_TEST_CASE(DataSpaceTest) {
  ...
  BOOST_CHECK_NE(space.getId(false), space2.getId(false));
...
BOOST_AUTO_TEST_CASE(HighFiveInspect) {
  ...
  // meta
  BOOST_CHECK(ds.getObjectType() == ObjectType::Dataset);  // internal
  BOOST_CHECK(ds.getObjectInfo().getHardLinkRefCount() == 1);
}
...
BOOST_AUTO_TEST_CASE(HighFiveFilename) {
  ...
  BOOST_CHECK_EQUAL("getFileName.h5", file.getFileName());
}
...
BOOST_AUTO_TEST_CASE(HighFiveCheckIdFunctionality) {
  ...
  BOOST_CHECK_EQUAL(file.getId(false), file.getFileId(false)); // getFileId() uses HDF5 native functionality
  BOOST_CHECK_EQUAL(file.getId(false), group.getFileId(false));
  BOOST_CHECK_EQUAL(file.getId(false), dataset.getFileId(false));
  BOOST_CHECK_EQUAL(file.getId(false), attr.getFileId(false));

  t.commit(file, "new_type1");

  BOOST_CHECK_EQUAL(file.getId(false), t.getFileId(false));
  ...
  BOOST_CHECK_EQUAL(file2.getIdRefCount(), 2);
  File file3 = File::FromId(file.getId(false), true);
  BOOST_CHECK_EQUAL(file3.getIdRefCount(), 3);
}
...
BOOST_AUTO_TEST_CASE(HighFiveSoftLink) {
  ...
  BOOST_CHECK(dset_out.getLinkInfo().getLinkType() == LinkType::Soft);
  ...
  BOOST_CHECK(group_out.getLinkInfo().getLinkType() == LinkType::Soft);

  DataSet group_out_dset = group_out.getDataSet("data");
  BOOST_CHECK(group_out_dset.isValid());
...
BOOST_AUTO_TEST_CASE(HighFiveHardLink) {
  ...
  BOOST_CHECK(dset_out.getLinkInfo().getLinkType() == LinkType::Hard);
  ...
  BOOST_CHECK(group_out.getLinkInfo().getLinkType() == LinkType::Hard);
...
BOOST_AUTO_TEST_CASE(HighFiveExternalLink) {
  ...
  BOOST_CHECK(group2.getLinkInfo().getLinkType() == LinkType::Hard);
  BOOST_CHECK_EQUAL(group2.getTargetPath(), "/path/to");
  BOOST_CHECK_EQUAL(group2.getPath(), "/path/to");
  BOOST_CHECK(!group2.getFileName().empty());
