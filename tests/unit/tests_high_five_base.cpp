...
BOOST_AUTO_TEST_CASE(HighFiveBasic) {
    ...
    Attribute attr = file.createAttribute<double>(ATTRIBUTE_NAME, DataSpace(1));

    BOOST_CHECK_EQUAL(file.getFileName(), FILE_NAME);
    BOOST_CHECK_EQUAL(attr.getFileName(), FILE_NAME);   // just to be sure that attribute also returns filename
    ...
    DataSet dataset_size_t = file.createDataSet<size_t>(DATASET_NAME + "_size_t",
                                                        dataspace);
    BOOST_CHECK_EQUAL(dataset_size_t.getFileName(), FILE_NAME);
}
