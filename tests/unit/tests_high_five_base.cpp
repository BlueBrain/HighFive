...
BOOST_AUTO_TEST_CASE(HighFiveCheckIdFunctionality) {
    ...
    // Check object's operators
    File file3("getFileName_other.h5", File::ReadWrite | File::Create | File::Truncate);
    Group group3 = file3.createGroup("group");
    DataSpace dataSpace3 = DataSpace(3);
    DataSet dataset3 = group3.createDataSet<int>("data", dataSpace3);
    Attribute attr3 = dataset3.createAttribute<int>("attr", dataSpace3);

    BOOST_CHECK(file == file2);
    BOOST_CHECK(file2 != file3);
    BOOST_CHECK(group == group2);
    BOOST_CHECK(group2 != group3);
    BOOST_CHECK(dataSpace == dataSpace2);
    BOOST_CHECK(dataSpace2 != dataSpace3);
    BOOST_CHECK(dataset == dataset2);
    BOOST_CHECK(dataset2 != dataset3);
    BOOST_CHECK(attr == attr2);
    BOOST_CHECK(attr2 != attr3);
}
